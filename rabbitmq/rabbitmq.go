// package github.com/lidaqi001/rabbitmq provides a RabbitMQ broker
package rabbitmq
...
func (r *rbroker) Publish(topic string, msg *broker.Message, opts ...broker.PublishOption) error {
	...
	if options.Context != nil {
		...
		if value, ok := ctx.Value(DeliveryMode{}).(uint8); ok {
			m.DeliveryMode = value
		}
		if value, ok := ctx.Value(Priority{}).(uint8); ok {
			m.Priority = value
		}
		if value, ok := ctx.Value(Expire{}).(int); ok {
			m.Expiration = strconv.Itoa(value)
		}
	}
...
func (r *rbroker) Subscribe(topic string, handler broker.Handler, opts ...broker.SubscribeOption) (broker.Subscriber, error) {
	...
	/**	@auth liqi lidaqi962464@qq.com	*/
	if val, ok := ctx.Value(isDelayQueueKey{}).(bool); ok && val {
