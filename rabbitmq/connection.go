...
func (r *rabbitMQConn) Publish(exchange, key string, msg amqp.Publishing) error {
	...
	if msg.Expiration != "" {
		key = DelayPrefix + key
		if err := r.ExchangeChannel.BindQueue(DelayQueue, key, r.exchange.Name, amqp.Table{});
			err != nil {
			return err
		}
	}
...
func (r *rabbitMQConn) declareDelay() error {
	...
	err = delayerChannel.DeclareDurableExchange(DelayExchange.Name)
	if err != nil {
		return err
	}
	...
	// 声明延时队列
	// 并将死信消息设置投递至延时交换机
	err = r.ExchangeChannel.DeclareDurableQueue(DelayQueue, amqp.Table{
